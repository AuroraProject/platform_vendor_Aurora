...
function auroraremote()
{
    ...
    local AURORA_USER=$(git config --get review.review.lineageos.org.username)
    if [ -z "$AURORA_USER" ]
    then
        git remote add aurora ssh://review.lineageos.org:29418/$PFX$PROJECT
    else
        git remote add aurora ssh://$AURORA_USER@review.lineageos.org:29418/$PFX$PROJECT
    fi
