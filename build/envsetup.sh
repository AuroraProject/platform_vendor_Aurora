...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Aurora model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch aurora_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
function Auroraremote()
{
    ...
    then
        ...
        local PFX="AuroraProject/"
    else
        local PROJECT=$REMOTE
    fi

    local AURORA_USER=$(git config --get review.review.AuroraProject.org.username)
    if [ -z "$AURORA_USER" ]
    then
        git remote add Aurora ssh://review.AuroraProject.org:29418/$PFX$PROJECT
    else
        git remote add Aurora ssh://$AURORA_USER@review.AuroraProject.org:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/AuroraProject/$PROJECT
...
function auroragerrit() {
    ...
    local user=`git config --get review.review.AuroraProject.org.username`
...
function aurorarebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "AuroraProject Gerrit Rebase Usage: "
    ...
    git fetch "http://review.AuroraProject.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
