...
- auroragerrit:   A Git wrapper that fetches/pushes patch from/to LineageOS Gerrit Review.
- aurorarebase:   Rebase a Gerrit change and push it again.
- auroraremote:   Add git remote for LineageOS Gerrit Review.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/aurora-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.aurora.device | grep -q "$aurora_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $aurora_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function auroraremote()
{
    ...
    git remote rm aurora 2> /dev/null
    ...
    local aurora_USER=$(git config --get review.review.auroraos.org.username)
    if [ -z "$aurora_USER" ]
    then
        git remote add aurora ssh://review.auroraos.org:29418/$PFX$PROJECT
    else
        git remote add aurora ssh://$aurora_USER@review.auroraos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'aurora' created"
}
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.aurora.device | grep -q "$aurora_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        auroraremote
        git push aurora HEAD:refs/heads/'$1'
    fi
    '
}

function auroragerrit() {
    ...
    local user=`git config --get review.review.auroraos.org.username`
...
function aurorarebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      aurorarebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.auroraos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.aurora.device | grep -q "$aurora_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
