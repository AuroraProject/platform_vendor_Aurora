...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from Aurora Github (http://github.com/Aurora)." % device)
...
def is_in_manifest(projectpath):
    ...
    # ... and don't forget the aurora snippet
    try:
        lm = ElementTree.parse(".repo/manifests/aurora.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('Aurora/%s already fetched to %s' % (repo_name, repo_target))
            continue

        print('Adding dependency: Aurora/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = {
            "path": repo_target,
            "remote": "github",
            "name": "Aurora/%s" % repo_name,
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/aurora.dependencies'
    syncable_repos = []
    verify_repos = []
...
print("Repository for %s not found in the Aurora Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
